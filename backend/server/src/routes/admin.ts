  ...
  asyncHandler(async (req: AuthRequest, res: express.Response) => {
    const [
      ...
      User.find()
        .sort({ createdAt: -1 })
        .limit(5)
        .select("userName schoolName email createdAt"),
      Product.find()
        .sort({ createdAt: -1 })
        .limit(5)
  ...
  asyncHandler(async (req: AuthRequest, res: express.Response) => {
    ...
    const [products, total] = await Promise.all([
      Product.find({ isReported: true })
        .populate(
          "sellerId",
          "userName profilePicture email schoolName firstName lastName university"
        )
        .populate(
          "reportedBy",
          "userName profilePicture email firstName lastName"
        )
        .sort({ updatedAt: -1 })
        .skip(skip)
        .limit(limitNum),
