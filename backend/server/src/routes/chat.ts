...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: express.Response) => {
    ...
    // If no product ID provided, find one from the seller
    if (!productIdToUse) {
      ...
      if (sellerProduct) {
        productIdToUse = sellerProduct._id;
      }
      // If seller has no products, we'll create chat without product (for admin chats)
    }

    // If we have a product ID, validate it
    if (productIdToUse) {
    ...
    // Find or create chat (model handles duplicate checking)
    const chat = await Chat.findOrCreateChat(buyerId, sellerId, productIdToUse);
