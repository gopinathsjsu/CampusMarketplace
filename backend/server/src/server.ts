...
const app = express();
const PORT = process.env.PORT || 5001; // Using 5001 because macOS uses 5000 for AirPlay
...
app.use(
  morgan(
    ':method :url :status :res[content-length] - :response-time ms ip=:remote-addr ua=":user-agent" body=:body'
  )
);
...
app.use("/api", (req, _res, next) => {
  console.log(
    `[api] ${req.method} ${req.originalUrl} ip=${req.ip} cl=${
    ...
    if (layer.route && layer.route.path) {
      ...
      layer.handle.stack.forEach((sub: any) => {
        if (sub.route && sub.route.path) {
          const methods = Object.keys(sub.route.methods).map((m: string) =>
            m.toUpperCase()
          );
...
const startServer = async () => {
  try {
    ...
    app.listen(PORT, () => {
      ...
      console.log(
        `📚 Campus Marketplace API ready at http://localhost:${PORT}/api`
      );
    });
  } catch (error) {
